...
{
    public function syncInvoices(): array|string
    {
        ...
        try {
            ...
            foreach ($invoices as $invoice) {
                ...
                // Create customer first to get the local customer_id
                $customerCompanyId = $resolveCompanyId($invoice['customer']['company_id'] ?? null);
                $customer = Customer::updateOrCreate([
                    'customer_id' => $invoice['customer_id'],
                ], [
                    'customer_name' => $invoice['customer']['customer_name'] ?? $invoice['customer_name'],
                    'type' => $invoice['customer']['type'] ?? 'individual',
                    'customer_id' => $invoice['customer_id'],
                    'customer_TIN' => $invoice['customer']['customer_TIN'] ?? null,
                    'customer_phone' => $invoice['customer']['customer_phone'] ?? null,
                    'customer_address' => $invoice['customer']['customer_address'] ?? $invoice['customer_address'],
                    'vat_customer_payer' => $invoice['customer']['vat_customer_payer'] ?? 0,
                    'company_id' => $customerCompanyId ?? $companyId,
                ...
                foreach ($invoice['invoice_items'] as $invoiceItem) {
                    ...
                    $productExists = ! $productId || Product::whereKey($productId)->exists();
                    ...
                    InvoiceItem::updateOrCreate([
                        ...
                        'invoice_id' => $c->id,
                        'product_id' => $productExists ? $productId : null,
                        'item_designation' => $invoiceItem['item_designation'],

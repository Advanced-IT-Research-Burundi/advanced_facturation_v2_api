...
class StockMovementController extends Controller
{
     ...
     * Get movement types
     */
    public function getMovementTypes($warehouseId)
    {
    ...
    public function movements(Request $request, $warehouseId)
    {
        ...
        DB::beginTransaction();
        try {
            // Default OBR status to PENDING
            $data = $validated;
            $data['obr_submission_status'] = 'PENDING';
            ...
            if (in_array($validated['item_movement_type'], self::ENTRY_TYPES)) {
                ...
                // For exits, check stock availability? (Optional but recommended, though validation might handle it in bulk methods)
                // For now, allow negative stock or assume checks done elsewhere? 
                // Creating a single movement usually implies we should check. 
                // But let's stick to the existing logic which didn't check in this function explicitly.
                $warehouseProduct->quantity = ($warehouseProduct->quantity ?? 0) - $validated['item_quantity'];
            }
